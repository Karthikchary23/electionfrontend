...
const AddCandidate = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://epbackend.onrender.com/positions")
  ...
  const fetchCandidates = () => {
    axios
      .get("https://epbackend.onrender.com/candidates")
  ...
  const handleSubmit = (e) => {
    ...
    const url = editingCandidateId
      ? `https://epbackend.onrender.com/candidates/${editingCandidateId}`
      : "https://epbackend.onrender.com/candidates";
  ...
  const handleDeleteCandidate = (id) => {
    ...
    if (isConfirmed) {
      axios
        .delete(`https://epbackend.onrender.com/candidates/${id}`)
        .then(() => {
          ...
          {isModalOpen && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="border-white border-[2px] [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#63e_100%)] 	 rounded-lg p-6 max-w-md w-full">
                <h2 className="text-xl text-white flex items-center justify-center font-bold mb-4">
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 m-2 ">
              {candidates.map((candidate) => (
                ...
                >
                  <img
                    src={`https://epbackend.onrender.com/uploads/${candidate.image}`}
                    alt={candidate.name}
                    className="w-full h-1/2 object-cover mb-4 rounded-md"
                  />
                  <h3 className="text-ms text-white font-bold">
                    {candidate.name}
                  </h3>
                  <p className="text-xs text-white font-semibold">
                    Position: <span>{candidate.position}</span>
                  </p>
                  <p className="text-xs text-white font-semibold">
                    Area: <span>{candidate.area}</span>
                  </p>
                  <p className="text-xs text-white font-semibold">
                    Number of Votes: <span>{candidate.vote_count}</span>
                  </p>
